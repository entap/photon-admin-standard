...
<div class="row">
	...
	<div class="col-9">
		...
		<?php if (isset($dev)) { ?>
			<p class="text-right">
				<a href="property.php?action=dev_edit&property_group_id=<?= $property_group['id'] ?>" class="btn btn-secondary" data-toggle="remote-modal">プロパティを作成</a>
				<a href="property.php?action=dev_group&id=<?= $property_group['id'] ?>" class="btn btn-secondary" data-toggle="remote-modal">グループを編集</a>
				<a href="property.php?action=dev_group_delete&id=<?= $property_group['id'] ?>" class="btn btn-secondary" onclick="return confirm('本当に削除しますか？');">グループを削除</a>
			</p>
		<?php } ?>
				...
				<tr>
					...
					<td>
						<div class="text-center">
							<?php if (isset($dev)) { ?>
								<a href="property.php?action=dev_edit&id=<?= $record['id'] ?>" class="btn btn-secondary btn-sm" data-toggle="remote-modal">編集</a>
							<?php } else { ?>
